...
def network2dot(nxDigraph):
    ...
    dot.node_attr['label'] = '' 
    dot.node_attr['width'] = '0.3' 
    # dot.edge_attr['fontsize'] = '20pt'
    
    for (u,v) in nxDigraph.edges():
        att = nxDigraph[u][v]
        dot.edge(str(u),str(v),label=f'{att["weight"]}',fontsize='40',penwidth='4',arrowsize='2')
...
for gt in range(args.ngenomes,args.ngenomes+1):
    ...
    for range_start in [10000]:  # range(0,min_length,range_increment):
        # progress printing
        ...
        leftNode = 1755
        rightNode = 1950
        # getting the subgraph of interest
        subgraphNodesFromLeft = nx.shortest_path(dbGraph_nx,leftNode).keys()
        subgraphNodesFromRight = nx.shortest_path(dbGraph_nx.reverse(copy=True),rightNode).keys()
        intersectionNodes = set(subgraphNodesFromLeft) & set(subgraphNodesFromRight)
        dbGraph_nx = nx.induced_subgraph(dbGraph_nx, intersectionNodes)
